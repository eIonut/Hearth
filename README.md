...
**Backup** — your data (snippets, notes, learnings, workflows, templates, patches) is gitignored, so it never ships with the app repo — which also means a lost laptop or fresh clone loses it. This tab points that data at a backup destination **you** own. Pick which collections to include (env presets, backups and local settings are never included); a **secret scan** flags anything that looks like a key or token before it leaves your machine. Two destinations, same data:
