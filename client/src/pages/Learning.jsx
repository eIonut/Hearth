...
function ItemForm({ onSaved, onCancel }) {
  ...
  return (
    <div className="card form-card">
      ...
      <label>What do you want to learn?<input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="React Server Components" /></label>
      <label>Link (article, video, docs)<input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://…" /></label>
      <label>Tags<input value={tags} onChange={(e) => setTags(e.target.value)} placeholder="react, ai" /></label>
      <label>Notes<textarea rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Why this matters / what to focus on" /></label>
              ...
              {colItems.map((item) => (
                <div className="card compact" key={item.id}>
                  <div className="row space-between">
                    ...
                    <button className="btn small danger" onClick={() => remove(item)}>✕</button>
                  </div>
                  {item.url && <a href={item.url} target="_blank" rel="noreferrer" className="small-text">{item.url}</a>}
                  {item.tags.length > 0 && (
                    <div className="small-text">{item.tags.map((t) => <span className="tag" key={t}>{t}</span>)}</div>
                  )}
                  {item.notes && <div className="muted small-text">{item.notes}</div>}
                  <div className="row">
                    {col.id !== 'queued' && <button className="btn small" onClick={() => setStatus(item, col.id === 'done' ? 'learning' : 'queued')}>←</button>}
                    {col.id !== 'done' && <button className="btn small primary" onClick={() => setStatus(item, col.id === 'queued' ? 'learning' : 'done')}>→</button>}
                  </div>
                </div>
              ))}
            </div>
