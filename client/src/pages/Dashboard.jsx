...
function ProjectForm({ initial, onSaved, onCancel }) {
  ...
  return (
    <div className="card form-card">
      ...
      <label>Preview URLs (one per line, "service: url" — opens inside the hub)
        <textarea rows={2} value={previewsText} onChange={(e) => setPreviewsText(e.target.value)} placeholder={'web: localhost:4000\napi: localhost:5000/docs'} />
      </label>
      <label>Links (one per line, "name: url" — repo, MRs, pipelines, docs; embeds in the hub when the site allows it, otherwise opens a new tab)
        <textarea rows={2} value={linksText} onChange={(e) => setLinksText(e.target.value)} placeholder={'repo: https://github.com/you/my-app\nMRs: https://gitlab.company.com/team/my-app/-/merge_requests'} />
      </label>
...
function LinkButton({ project, link }) {
  ...
  if (embeddable) {
    return (
      <button className="btn small" onClick={() => openPreview(`${project.name}/${link.name}`, link.url)}>
        {link.name}
      </button>
    );
  }
...
function LogPanel({ target, onClose }) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const data = await api(`/services/logs?projectId=${target.projectId}&service=${encodeURIComponent(target.service)}`);
